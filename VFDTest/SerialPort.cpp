...

{
	...
	for (;;)
	{
		...
		// run. My solution is to start a forever loop and to
		// break out of it when I have processed all of the
		// data available.  Be careful with this approach an
		// be sure your loop will exit.
		// My reasons for this are not as clear in this sample
		// as it is in my production code, but I have found this
		// solutiion to be the most efficient way to do this.
		...
		CString str = _T(""),strData = _T("");
		if (bRead)
		{
			for (int i = 0; i < BytesRead; i++ )
			{
				str.Format(_T("%02x"),(unsigned char)*(readBuff + i));
				strData += str;
			}
		}
		//CString strLog;
		...
		//if(SFile.Open( _T("ReceiveString.txt"), CFile::modeCreate | CFile::modeNoTruncate | CFile::modeWrite ), &fileException )
		//{
		...
		// notify parent that a string was received
		//if (strData.Mid(2,2) == _T("01") || strData.Mid(2,2) == "03")// 接收到的数据为继电器查询返回数据或者仪表返回数据，则通知主界面进行相应操作
		{
